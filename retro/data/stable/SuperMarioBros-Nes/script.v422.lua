...
function dying_penalty ()
    ...
    elseif data.state == 8 and data.yposHi == 1 and data.yposLo > 176 and is_dying == false then
        local reward = -5
        --- increase the die_pit penalty on stage 1-3 and 4-3
        --- or else jumps into pit
        if (data.levelHi == 0 and data.levelLo == 2) then
            reward = -12.5
        end
...
function sum_reward ()
    return xscrollLo_reward() + dying_penalty() + lives_tracker() + progress_tracker() + stage4_4_penalty() + stage7_4_penalty() + checkpoint_tracker()
end
