...
function stage4_4_penalty ()
    ...
    if data.levelHi == 3 and data.levelLo == 3 then
        -- loop check
        if data.time ~= 0 and is_looped then
            local reward = -7.5
            data.time = 0
        ...
        elseif ((data.xscrollHi == 5 and data.xscrollLo >= 210) or (data.xscrollHi == 6 and data.xscrollLo <= 20)) and (data.yposHi == 1 and data.yposLo <= 128) and data.time ~= 0 then
            local reward = -7.5
            data.time = 0
            if debug then print('stage4_4_penalty: ', reward) end
            return reward
...
function progress_tracker ()
    -- if progress is incrementing or go back the start of the stage
...
function stage_done ()
    -- stop at stage 5-1
    if data.levelHi == 4 and data.levelLo == 0 then
        return true
    else
        return false
    end
